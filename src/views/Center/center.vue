    ...
    <div class="center-view">
        ...
        >
            ...
            <img
                ...
                v-show="userinfo.mobile"
            />
            <div class="nick-name" v-show="!userinfo.mobile">立即登录</div>
            <div class="nick-name" v-show="userinfo.mobile">{{userinfo.mobile}}</div>
        </div>
...
export default {
    ...
    created () {
        ...
        if(jwt){
            this.$https.get("/v1/info").then((ret) => {
                console.log(ret)
                if (ret.code == 0) {
...
.center-view .my-balance .value, .center-view .my-card .value {
    text-align: right;
    color: #797d82;
    padding-right: 14px;
}
